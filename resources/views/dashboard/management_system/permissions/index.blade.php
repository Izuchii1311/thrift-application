...
{{-- @section('title', $menu_path ? $menu_path->menu_name : 'Management Permission') --}}
@section('title', 'Management Permission')
@section('management-permission', 'here')
                    ...
                    <div class="card-toolbar">
                        ...
                        <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                            ...
                            @if ($menu_access['can_create'])
                                ...
                                <i class="ki-outline ki-plus fs-2"></i>Tambah Permission Baru</button>
                            @endif
                        </div>
                                        ...
                                        <form id="form-data" class="form" action="">
                                            ...
                                            <div class="d-flex flex-column scroll-y px-5 px-lg-10" id="form_modal_scroll" data-kt-scroll="true" data-kt-scroll-activate="true" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#form_modal_header" data-kt-scroll-wrappers="#form_modal_scroll" data-kt-scroll-offset="300px" >
                                                ...
                                                <div class="fv-row mb-7">
                                                    ...
                                                    <input type="text" name="permission_name" id="permission_name" class="form-control form-control-solid mb-3 mb-lg-0" placeholder="Nama Permission"/>
                                                </div>

                                                {{-- Input data --}}
                                                <div class="fv-row mb-7">
                                                    <label class="required fw-semibold fs-6 mb-2">Permission Action</label>
                                                    <input type="text" name="permission_action" id="permission_action" class="form-control form-control-solid mb-3 mb-lg-0" placeholder="Permission Action"/>
                                                    <div class="text-muted mt-2">Permission action seperti (view, create, update, delete dan sebagainya).</div>
                                                </div>

                                                {{-- Input data --}}
                                                <div class="fv-row mb-7">
                                                    <label class="required fw-semibold fs-6 mb-2">Menu</label>
                                                    <select class="form-select mb-2" data-control="select2" data-hide-search="true" data-placeholder="Pilih Parent Menu" name="menu_id" id="menu_id">
                                                        <!-- select2 -->
													</select>
                                                    <div class="text-muted mt-2">Menu wajib dipilih, permission yang dibuat ini akan ditetapkan kedalam menu.</div>
                                                </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="table">
                        <thead>
                            <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-125px">Permission Action</th>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            table = $('#table').DataTable({
                ...
                ajax: {
                    url: '{{ route("management-permission.indexJson") }}',
                    type: 'POST',
                    data: function (d) {
                        d._token = "{{ csrf_token() }}";
                        d.search = $('#search').val();
                ...
                columns: [
                    ...
                    { data: 'permission_name',  orderable: true },
                    { data: 'menu_name',        orderable: true },
                    { data: 'permission_action',orderable: true, className: 'text-center' },
        ...
        function tambahData() {
            $('#form-data').attr('action', '{{ route("management-permission.store") }}');

            $('#form-data')[0].reset();
            $('#form-data select').val(null).trigger("change");

            $('#form_modal_title').text('Form Tambah Permission Baru');
            $('#permission_name, #permission_action, #menu_id, #description').val("");

            $('#form_modal').modal('show');
        }

        function editData(encryptedId) {
            $('#form-data').attr('action', "{{ url('management-permission/update') }}" + "/" + encryptedId);

            $('#form_modal_title').text('Edit Data Permission');
            showLoading();

            $.ajax({
                url: "{{ url('management-permission/detail/json') }}" + "/" + encryptedId,
                type: 'POST',
                data: { _token: "{{ csrf_token() }}" },
                success: function(result) {
                    const menu = result.response;

                    $('#permission_name').val(menu.permission_name);
                    $('#permission_action').val(menu.permission_action);
                    $('#description').val(menu.description);

                    if ($('#menu_id option[value="' + result.response.menu_id+ '"]').length > 0) {
                        $('#menu_id').val(result.response.menu_id).trigger('change');
                    }
                ...
                error: function(result) {
                    alertResultError(result, "Error, Data Permission gagal didapatkan.");
                }
            });
        }
        ...
        function deleteData(encryptedId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ url('management-permission/delete') }}" + "/" + encryptedId,
                        type: 'DELETE',
                        data: { _token: "{{ csrf_token() }}" },
                        success: function (res) {
                            const metadata = res?.metadata;
        ...
        function getMainMenusOption(reset = false) {
            $.ajax({
                url: "/options/main-menus",
                method: "POST",
                data: {
                    _token: "{{ csrf_token() }}"
                },
                success: function(result) {
                    $('#menu_id').empty().append('<option></option>');
